...
//   video_player.innerHTML = `<div class='offline'> <h1> ${online ? "Online" : "OFFline"} </h1> </div>`;
// }, 3000);
  ...
  const mainVideo = video_player.querySelector(".main-video"),
    ...
    logo = video_player.querySelector(".logo");
...
let getTime = localStorage.getItem("currentVideoTime");
mainVideo.currentTime = getTime
});
  ...
  mainVideo.addEventListener("timeupdate", (e) => {
    let setcurrentVideoTime = localStorage.setItem("currentVideoTime" ,mainVideo.currentTime);
  ...
  video_player.addEventListener("mousemove", () => {
    ...
    // if(logo.classList.contains("active_logo")){
    // logo.classList.remove("active_logo");
